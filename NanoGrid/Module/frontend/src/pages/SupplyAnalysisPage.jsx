...
function SupplyAnalysisPage() {
  ...
  const loadResources = async () => {
    try {
      setLoading(true)
      const response = await supplyService.getResources()
  ...
  const loadModels = async () => {
    try {
      const response = await supplyService.getModels()
      if (response.data.success) {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this energy resource?')) return
    
    try {
      await supplyService.deleteResource(id)
      loadResources()
    } catch (err) {
  ...
  const handleViewMetadata = async (id) => {
    try {
      const response = await supplyService.extractMetadata(id, includeWeather)
      if (response.data.success) {
  ...
  const runAnalysis = async () => {
    ...
    try {
      setAnalyzing(true)
      const response = await supplyService.analyze(selectedResource, {
      ...
      {loading ? (
        ...
        <Paper sx={{ p: 4, textAlign: 'center' }}>
          <Typography variant="h6" color="text.secondary" gutterBottom>
            No energy resources found
          </Typography>
          <Typography variant="body2" color="text.secondary" paragraph>
            Click "Add Energy Resource" to get started
          </Typography>
          ...
          {analysisResults && (
            <Paper sx={{ p: 3, mt: 3 }}>
              ...
              <Grid container spacing={2}>
                ...
                {analysisResults.has_weather_data && (
                  <Grid item xs={12}>
                    <Alert severity="success">Weather data included in analysis</Alert>
                  </Grid>
                )}
              </Grid>
            </Paper>
        ...
        <DialogContent>
          ...
          <FormGroup>
            <FormControlLabel
              ...
              label="Include Weather Data for Accurate Forecasting"
            />
          </FormGroup>
        </DialogContent>
        <DialogActions>
...
export default SupplyAnalysisPage
